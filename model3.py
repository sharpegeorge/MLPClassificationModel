...
# Ignore the first 5 columns and the last column
...
# Reshape the features for sequential input
features = features.reshape((len(features), seq_length, -1))
...
X_train_resampled = X_train_resampled.reshape((len(X_train_resampled), seq_length, -1))

# Build the model with dropout and L2 regularization
model = Sequential()
model.add(LSTM(units=64, return_sequences=True, kernel_regularizer=l2(0.01), input_shape=(seq_length, features.shape[2])))
model.add(Dropout(0.5))  # Add dropout layer
model.add(LSTM(units=32, kernel_regularizer=l2(0.01)))  # Adjusted for return_sequences
model.add(Dropout(0.5))  # Add dropout layer
